<script>
	...
	let showMarker = true;
	let showMarkerDistorsion = true;
	let markerSelector = 0;
	let markerDimension = 1;
	...
	onMount(() =>
		alert(
			'Welcome to the Merge Tree Playground for marker distorsion, please be careful while adjusting the settings as it can be very CPU INTENSIVE.'
		)
	);
	...
	<div>
		...
		>
			...
			<label
				for="markerDimension"
				class="block mb-2 text-sm font-medium text-gray-900 dark:text-gray-300"
				>Marker width and height:</label
			>
			<input
				type="number"
				id="markerDimension"
				name="markerDimension"
				min="1"
				max="1.9"
				step=".1"
				bind:value={markerDimension}
				class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500"
			/>
